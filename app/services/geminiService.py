...
class GeminiService:
    ...
    # def generate_intent_prompt(self, user_input: str):
    #     categories = "\n".join([f"{k}: {v}" for k, v in INTENT_KEYWORDS.items()])
    #     prompt = f"""
    #     You are a smart intent detection system. Given a user's input, classify it into one of the following categories:

    #     {categories}

    #     User input: "{user_input}"

    #     Only reply with one of the category keys: {", ".join(INTENT_KEYWORDS.keys())}.
